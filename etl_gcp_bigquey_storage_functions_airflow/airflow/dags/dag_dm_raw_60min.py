...
"""DAG que orquestra, a cada hora, a carga nas tabelas RAW e o versionamento (SCD) das bases de atualizacao horaria."""
...
def table_querys_ref_job():
    # tabela de controle (espelho no BQ) que indica quais bases devem ser processadas e com qual frequencia
    query = """
        select dataset, raw_table as table_name
        from `gcp-project-id.control_etl_process.tb_ref_tables`
        where integration_bq = 1
            and update_frequency_min = 60;
    """
...
with models.DAG(
    ...
    for _, row in ref_tables.iterrows():
        ...
        start >> job_gcf_1_insert_raw_table >> job_data_quality >> job_gcf_2_scd_versioning
